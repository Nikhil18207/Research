...
nohup python3 constant_time_proxy.py --port 8001 --upstream http://localhost:8000 --bucket-ms 150 \
  > ../results/proxy_quantized.log 2>&1 &
disown
sleep 1
cd ../scripts
python3 ../drivers/rq1_tier_characterization.py \
  ...
  --out ../results/rq1_quantized_raw.csv
